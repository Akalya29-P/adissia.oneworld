...
export async function POST(req) {
  try {
    ...
    // Check for email duplicates as well (optional additional check)
    const emailSubmissions = Object.entries(submittedPhones).find(([key, date]) => 
      key.includes(email) && date === today
    );
    
    if (emailSubmissions) {
      console.log('🔄 Duplicate email submission detected:', email);
      return NextResponse.json({ 
        status: 'duplicate', 
        message: 'This email has already been used for a submission today. Please try again tomorrow.',
        submittedDate: today
      }, { status: 409 }); // HTTP 409 Conflict
    }

    // Save new submission (using both phone and email as keys for better tracking)
    submittedPhones[phone] = today;
    submittedPhones[`email_${email}`] = today;
